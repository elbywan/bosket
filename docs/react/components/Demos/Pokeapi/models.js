...
export class Item {
    ...
    display() {
        if(this.url && this.name) {
            return <span className="subcategory">{ this.name }</span>
        }

        const properties = []
        if(this.__image) {
            properties.push(<img src={ this.__image } alt="sprite" key="__pokesprite"/>)
        }
        ...
        return <div className={ this.__subsection ? "subsection" : "" }>{ properties }</div>
    }
}
    ...
    for(const prop in data) {
        ...
        if(data[prop] instanceof Array) {
            ...
            if(data[prop].url && data[prop].name) {
                ...
                data.__children.push({
                    display: () => <span className="subcategory">{ "" + name }</span>,
